...
class NiftiBoundingBox:
    ...
    def _check_zooms(self, verbose=False):
        ...
        if not np.allclose(zoom_array.min(axis=0), zoom_array.max(axis=0)):
            ...
            if verbose: print(f"Multiple different voxel sizes detected. Using isotropic voxel size = {self.voxel_size} (lowest resolution)")
        else:
            self.voxel_size = tuple(zoom_array[0])
            if verbose: print(f"All files appear to have the same voxel sizes: {self.voxel_size}. No reslicing needed.")
